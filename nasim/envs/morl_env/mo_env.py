...
class MONASimEnv(gym.Env):
    ...
    def step(self, action):
        ...
        """
        next_state, obs, objective_values, done, info = self.generative_step(
            self.current_state,
            action
        )
    ...
    def generative_step(self, state, action):
        ...
        next_state, mo_action_result = self.network.perform_multi_objective_action(
            state, action
        )
        obs = next_state.get_observation(
            action, mo_action_result, self.fully_obs
        )
        done = self.goal_reached(next_state)

        return next_state, obs, mo_action_result.objective_values, done, mo_action_result.info()
