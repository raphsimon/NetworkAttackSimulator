...
class ModifiedScenarioGenerator:
    ...
    **Host Configuration distribution**:
    ...
    def _generate_exploits(self, exploit_cost, exploit_probs):
        exploits = {}
        num_exploits = len(self.os) * len(self.services)
        exploit_probs = self._get_action_probs(num_exploits, exploit_probs)
        
        # We create one exploit per OS service and combination
        exploits_added = 0
        for os in self.os:
    ...
    def _generate_privescs(self, privesc_cost, privesc_probs):
        privescs = {}
        num_privesc = len(self.os) * len(self.processes)
        privesc_probs = self._get_action_probs(num_privesc, privesc_probs)

        # we create one privesc action per OS and process combination
        privescs_added = 0
        for os in self.os:
            for proc in self.processes:
                pe_name = f"pe_{proc}_{os}"
                privescs[pe_name] = {
